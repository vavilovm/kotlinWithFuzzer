...
import org.jetbrains.kotlin.bbf.bugfinder.util.filterNotLines
...
import org.jetbrains.kotlin.bbf.reduktor.util.containsChildOfType
...
data class BBFFile(val name: String, var psiFile: PsiFile, var ctx: BindingContext? = null) {
    ...
    fun unsafeReplacingOfPsiFile(newPsiFile: PsiFile) {
        this.psiFile = newPsiFile
        this.ctx = null
    }

    fun replacePsiFile(newPsiFile: PsiFile) = replacePsiFile(newPsiFile.text)

    fun replacePsiFile(newPsiFileText: String, checkCorrectness: Boolean = true) {
