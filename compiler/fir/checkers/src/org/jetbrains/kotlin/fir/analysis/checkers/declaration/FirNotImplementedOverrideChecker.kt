...
object FirNotImplementedOverrideChecker : FirClassChecker() {
    ...
    private fun FirClass<*>.unsubstitutedScope(context: CheckerContext) =
        this.unsubstitutedScope(context.sessionHolder.session, context.sessionHolder.scopeSession, withForcedTypeCalculator = true)

    // TODO: Collection extensions are not properly regarded as an override. Should not have a whitelist like this.
    private val COLLECTIONS = listOf("java.util", "kotlin.collections")
...
*/
                ...
                // TODO: Collection extensions are not properly regarded as an override.
                val originalPackage =
                    notImplementedFunction?.originalIfFakeOverride()?.dispatchReceiverType?.classId?.packageFqName?.asString()
                if (originalPackage in COLLECTIONS) continue
