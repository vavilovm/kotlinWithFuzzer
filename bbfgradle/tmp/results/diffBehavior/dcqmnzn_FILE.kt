...
fun z(x: Int, y: Int): String {
    var res = dispatch<Int>(IC(40)) + 4
var prope0: String
    if ((IC("ykrou").extensionValue<Collection<Int>>()).iterator().hasNext()){
println("""THEN""");

        if ((extension<Int>(IC({"OK"}))).equals("OK").or(res != 45)) {
            prope0 = "a"
        } else {
...
println("""ELSE""");
        ...
        prope0 = (extension<Int>(IC(1))).div((extension<Int>(IC(1))).div(x)).toString(3)
}
    return "foo"
}
...
fun doCall(): String {
    var prope0: IC
var box = underlying<Any>(IC(40)) == 2
...
class Kla1 {
    companion object Kla2 {
        val prope2 = (normal<Int>(IC(40))).div(-3293458409855928393).toString(extension<Int>(IC(40)))
    }
}
