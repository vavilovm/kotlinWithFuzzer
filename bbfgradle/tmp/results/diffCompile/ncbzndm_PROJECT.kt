...
open class Kla0 {
    ...
    inner class Kla1 : Kla0() {
        fun fu1(): String {
            ...
            if ((1UL.toUByte()).toDouble().isFinite()) return (A()).inlineMe({ "itmxh"}).toString()
            return "OK"
        }
    }
}
...
fun box833(): String {
    return (Pair<Short?, Kla0>(10, Kla0())).component2().fu0()
}

inline fun <Ty0> fu2(arg: Ty0, crossinline f: (String) -> Unit) {
    { print("zybfp")}()
}

fun box376(): String {
    val prope3 = (Kla0().fu0()).get(37463732).lowercase()
    var prope4 = (Kla0().prope0).subSequence(1808192167, 36960386).substring((2933243719407603662.toULong()).compareTo(1.toUByte()), 13)
    fu2((linkedMapOf<A, A>(Pair<A, A>(A(), A()))).size.toString(2132301893))  {a: String -> check(true, { "vofxc"})}

    return (mapOf<Char?, Double>(Pair<Char?, Double>('4', 0.03368164104064686))).size.toString(-13)
}
...
fun fu3(call: () -> String) : Kla2 = Kla2("qyuqv")
