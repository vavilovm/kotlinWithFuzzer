...
sealed class Result<TInput, TValue> {
    ...
    class ParseError<Outer, TValue>(val productionLabel: (String)?,
                                     val child: ParseError<(TInput)?, *>?,
                                     val rest: (TInput)?) : Result<TInput, TValue>() {}
}

fun box2() : (String)? {
   if( A(null,19).hashCode() != 229077302!!.unaryMinus()) "spfwo"
   if( A(239,19).hashCode() != (((239).times(31)).plus(94237251.unaryMinus()))) "fail"
   ...
   if( D(839693111).hashCode() != (239)) "fail"
